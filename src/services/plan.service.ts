...
export async function generateMonthlyPlan(brief: Brief): Promise<BrandData> {
    // Генерируем системный промт для генерации контент плана
    const sys = generatePlanSystemPrompt()
    // Генерируем промт на основе брифа
    ...
    return parseTextFromAi(raw)
}

/**
 * Создаем промт для картинок
 * @param brandData
 * @param category
 * @param subcategory
 */
export async function generateImagePrompts(brandData: BrandData, category?: string, subcategory?: string) {
    const prompts = Promise.all(brandData.contentPlan.calendar.map(async (item) => {
        const imagePrompt = generateImagePrompt({
            brandStrategy: brandData.brandStrategy,
            item,
            category: (category as keyof typeof promptTemplates) || "clothes",
            subcategory: subcategory || "outerwear"
        })
